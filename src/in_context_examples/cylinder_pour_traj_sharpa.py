...
_RENDER_EVERY = 8
_FRAMES_DIR   = os.environ.get("ISAAC_FRAMES_DIR", "/tmp/isaac_frames_cyl_pour_traj_sharpa")
import shutil as _shutil
if Path(_FRAMES_DIR).exists():
    _shutil.rmtree(_FRAMES_DIR)
Path(_FRAMES_DIR).mkdir(parents=True, exist_ok=True)

# Grasp / approach parameters (matching cylinder_pour)
...
try:
    ...
    if frames:
        _vid = str(Path(__file__).with_suffix("")) + "_sharpa.mp4"
