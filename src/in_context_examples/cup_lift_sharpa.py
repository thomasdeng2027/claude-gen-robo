...
Adapted to use the realkinova sharpa hand instead of the xhand.
Side grasp: EE Z → approach dir (robot→object), EE Y → world +Z (thumb up).

Run:
    python cup_lift_sharpa.py
"""
...
_IK_URDF_PATH = _REPO_ROOT + "/assets/realkinova_sharpa_hand/realkinova_sharpa_hand_right.urdf"
...
# Disable DLSS before any render products are created.
# On A100 GCP, DLSS downscales (640,480) → (320,240) internally; 240 < 300 minimum
# makes DLSS output all-black frames.  Switch to FXAA (no minimum resolution).
import carb as _carb
_cs = _carb.settings.get_settings()
_cs.set("/rtx/post/aa/op", 2)          # 2 = FXAA in Omniverse RTX
_cs.set("/rtx/post/dlss/execMode", 0)  # belt-and-suspenders: DLSS exec mode off
print(f"[aa] op={_cs.get('/rtx/post/aa/op')}  dlss_exec={_cs.get('/rtx/post/dlss/execMode')}", flush=True)
...
_RENDER_EVERY = 8
_FRAMES_DIR   = os.environ.get("ISAAC_FRAMES_DIR", "/tmp/isaac_frames_cup_lift_sharpa")
...
_GRASP_OFFSET_X_M          =  0.08 # world X offset from cup center (+ = in towards cup)
_GRASP_OFFSET_Y_M          = -0.01  # world Y offset from cup center (+ = away, - = toward robot)
_GRASP_OFFSET_Z_M          =  0.40 # world Z offset from cup center (+ = up, toward rim)
...
_INIT_HAND = {
    "right_thumb_CMC_FE":  0.0,
    "right_thumb_CMC_AA":  0.0,
    "right_thumb_MCP_FE":  0.0,
    "right_thumb_MCP_AA":  0.0,
    "right_thumb_IP":      0.0,
    "right_index_MCP_FE":  0.0,
    "right_index_MCP_AA":  0.0,
    "right_index_PIP":     0.0,
    "right_index_DIP":     0.0,
    "right_middle_MCP_FE": 0.0,
    "right_middle_MCP_AA": 0.0,
    "right_middle_PIP":    0.0,
    "right_middle_DIP":    0.0,
    "right_ring_MCP_FE":   0.0,
    "right_ring_MCP_AA":   0.0,
    "right_ring_PIP":      0.0,
    "right_ring_DIP":      0.0,
    "right_pinky_CMC":     0.0,
    "right_pinky_MCP_FE":  0.0,
...
_HAND_CLOSED = {
    "right_thumb_CMC_FE":  1.7,
    "right_thumb_CMC_AA":  0.0,
    "right_thumb_MCP_FE":  1.2,
    "right_thumb_MCP_AA":  0.0,
    "right_thumb_IP":      1.5,
    "right_index_MCP_FE":  1.4,
    "right_index_MCP_AA":  0.0,
    "right_index_PIP":     1.5,
    "right_index_DIP":     1.0,
    "right_middle_MCP_FE": 1.4,
    "right_middle_MCP_AA": 0.0,
    "right_middle_PIP":    1.5,
    "right_middle_DIP":    1.0,
    "right_ring_MCP_FE":   1.4,
    "right_ring_MCP_AA":   0.0,
    "right_ring_PIP":      1.5,
    "right_ring_DIP":      1.0,
    "right_pinky_CMC":     0.2,
    "right_pinky_MCP_FE":  1.4,
    "right_pinky_MCP_AA":  0.0,
    "right_pinky_PIP":     1.5,
    "right_pinky_DIP":     1.0,
}
...
_HAND_PARTIAL = {
    "right_thumb_CMC_FE":  0.7,
    "right_thumb_CMC_AA":  0.0,
    "right_thumb_MCP_FE":  0.5,
    "right_thumb_MCP_AA":  0.0,
    "right_thumb_IP":      0.6,
    "right_index_MCP_FE":  0.6,
    "right_index_MCP_AA":  0.0,
    "right_index_PIP":     0.6,
    "right_index_DIP":     0.4,
    "right_middle_MCP_FE": 0.6,
    "right_middle_MCP_AA": 0.0,
    "right_middle_PIP":    0.6,
    "right_middle_DIP":    0.4,
    "right_ring_MCP_FE":   0.6,
    "right_ring_MCP_AA":   0.0,
    "right_ring_PIP":      0.6,
    "right_ring_DIP":      0.4,
    "right_pinky_CMC":     0.1,
    "right_pinky_MCP_FE":  0.6,
    "right_pinky_MCP_AA":  0.0,
    "right_pinky_PIP":     0.6,
    "right_pinky_DIP":     0.4,
}
...
def _ee_link_world(stage, robot_prim=None):
    """Return world position of right_hand_C_MC — the cuRobo tool frame."""
    prefix = (_ROBOT_PRIM if robot_prim is None else robot_prim).rstrip("/") + "/"
    return _prim_world_pos(stage, prefix + "right_hand_C_MC")


def _knuckle_centroid(stage, robot_prim=None):
    prefix = (_ROBOT_PRIM if robot_prim is None else robot_prim).rstrip("/") + "/"
    knuckle_links = [
        "right_index_MCP_VL", "right_middle_MCP_VL",
        "right_ring_MCP_VL", "right_pinky_MCP_VL",
        "right_thumb_CMC_VL",
    ]
    wrist_links = ["right_hand_C_MC"]
    ...
    if len(knuckle_pos) >= 2:
        ...
        return centroid
    for ln in wrist_links + ["right_thumb_CMC_VL"]:
...
# add_default_ground_plane() creates the standard Isaac Sim ground + default lighting/sky.
# The manual GroundPlane(prim_path=...) skips the default lighting setup, causing black frames.
...
def _step(render_force=False):
    ...
    if render_force or (_frame_counter[0] % _RENDER_EVERY == 0):
        ...
        try:
            ...
            img = _rgb.get_data()
            if not _frame_diag_done[0]:
                _frame_diag_done[0] = True
                print(f"FRAME_DIAG type: {type(img)}", flush=True)
                if img is not None:
                    print(f"FRAME_DIAG raw:  shape={img.shape} dtype={img.dtype} "
                          f"min={img.min()} max={img.max()} mean={img.mean():.3f}", flush=True)
                    _has_npy = hasattr(img, 'numpy')
                    _dev = getattr(img, 'device', 'N/A')
                    print(f"FRAME_DIAG meta: has_numpy_method={_has_npy} device={_dev}", flush=True)
            if img is not None and img.size > 0:
                # Handle warp arrays (GPU-backed): .numpy() syncs & copies to CPU.
                # Plain numpy: ascontiguousarray is sufficient.
                ...
                if _frame_counter[0] == 8:  # log the cpu copy on the first rendered frame
                    print(f"FRAME_DIAG cpu:  shape={img_cpu.shape} dtype={img_cpu.dtype} "
                          f"min={img_cpu.min()} max={img_cpu.max()} mean={img_cpu.mean():.3f}", flush=True)
            ...
            if not _frame_diag_done[0]:
                _frame_diag_done[0] = True
                print(f"FRAME_DIAG: exception={_e}", flush=True)
    else:
        world.step(render=False)
...
def _log_finger_tips(label: str):
    ...
    links = {
        ...
        "thumb_base": "right_thumb_CMC_VL",
        "thumb_tip":  "right_thumb_fingertip",
        "index_base": "right_index_MCP_VL",
        "index_tip":  "right_index_fingertip",
        "mid_tip":    "right_middle_fingertip",
...
_ROBOT_CFG_DICT = {
    "robot_cfg": {
        "kinematics": {
            ...
            "base_link":        "base_link",
            "tool_frames":      ["right_hand_C_MC"],
            "grasp_contact_link_names": ["right_hand_C_MC"],
            "collision_link_names": [
                "base_link", "Shoulder_Link", "HalfArm1_Link", "HalfArm2_Link",
                "ForeArm_Link", "SphericalWrist1_Link", "SphericalWrist2_Link",
                "Bracelet_Link", "sharpa_mount_right", "right_hand_C_MC",
            ],
            "collision_spheres": {
                ...
                "sharpa_mount_right":   [{"center":[0., 0., 0.04],"radius":0.040}],
                "right_hand_C_MC":      [{"center":[0., 0., 0.02],"radius":0.050},
            ...
            "self_collision_ignore": {
                ...
                "SphericalWrist2_Link": ["Bracelet_Link", "sharpa_mount_right", "right_hand_C_MC"],
                "Bracelet_Link":        ["sharpa_mount_right", "right_hand_C_MC"],
                "sharpa_mount_right":   ["right_hand_C_MC"],
            },
...
# Saved to trajectories/cup_lift_sharpa/ at the end of execution.
_obj_traj: list[dict] = []
...
if _push_ik.success.any():
    ...
    _execute_segment("PushDown", _push_wps, _HAND_OPEN, _cur_cmd,
...
try:
    ...
    _traj_dir = Path(__file__).parent.parent.parent / "trajectories" / "cup_lift_sharpa"
...
# ── Encode video ───────────────────────────────────────────────────────────────
try:
    import glob as _glob, av as _av, imageio.v2 as _iio
    frames = sorted(_glob.glob(os.path.join(_FRAMES_DIR, "frame_*.png")))
    if frames:
        _vid = os.path.join(os.path.dirname(os.path.abspath(__file__)), "cup_lift_sharpa.mp4")
        _img0 = _iio.imread(frames[0])[:, :, :3]
        _h, _w = _img0.shape[:2]
        with _av.open(_vid, mode="w") as _con:
            _st = _con.add_stream("libx264", rate=30)
            _st.width = _w; _st.height = _h; _st.pix_fmt = "yuv420p"
            for _fp in frames:
                _fr = _av.VideoFrame.from_ndarray(_iio.imread(_fp)[:, :, :3], format="rgb24")
                for _pkt in _st.encode(_fr): _con.mux(_pkt)
            for _pkt in _st.encode(): _con.mux(_pkt)
